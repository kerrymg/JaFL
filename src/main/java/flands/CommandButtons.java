...
implements ActionListener, LayoutManager {
    ...
    private boolean includeTopMargin = true;
    private boolean includeBorderMargin = true;
     ...
     * @see #RESET_BUTTON
     */
    private static final int APPLY_BUTTON = 1 << 0;
    /**
     ...
     * @see #RESET_BUTTON
     */
    private static final int CLOSE_BUTTON = 1 << 2;
    /**
     ...
     * @see #CLOSE_BUTTON
     */
    private static final int RESET_BUTTON = 1 << 4;
    /**
     * Code for the standard 'Help' button.  Displays help for the
     * dialog.
     */
    private static final int HELP_BUTTON = 1 << 5;

    /* *******************************
     * Action Commands for the buttons
     ******************************* */
    private static final String applyCommand = "c.a";
    static final String cancelCommand = "c.x";
    private static final String closeCommand = "c.c";
    private static final String helpCommand = "c.h";
    static final String okCommand = "c.o";
    private static final String resetCommand = "c.r";
     ...
     */
    static final int OK_CANCEL = OK_BUTTON | CANCEL_BUTTON;
     ...
     */
    private static final int APPLY_CLOSE = APPLY_BUTTON | CLOSE_BUTTON;
     ...
     * @see #APPLY_CLOSE
     */
    private static final int APPLY_RESET_CLOSE = APPLY_CLOSE | RESET_BUTTON;
    ...
    public static CommandButtons createRow(int buttons, ActionListener listener) {
		...
		for (int aDefaultOrder : DefaultOrder)
			if ((buttons & aDefaultOrder) != 0)
				row.addButton(aDefaultOrder);
	...
	private void addButton(int buttonCode) {
		insertButton(buttonCode, -1);
    }
    /**
     * Add one of the standard buttons to the row.
     * @param index the index at which the button should be inserted.
     */
	private void insertButton(int buttonCode, int index) {
     ...
     */
	private void insertButton(AbstractButton button, int index) {
		button.addActionListener(this);
		doAddButton(button, index);
    }
     ...
     */
	JButton getButton(int buttonCode) {
		int i = codeToIndex(buttonCode);
		if (i < 0) return null;
     ...
     */
	private boolean getIncludeTopMargin() { return includeTopMargin; }
    /**
     ...
     */
	private boolean getIncludeBorderMargin() { return includeBorderMargin; }
    /**
    ...
    private int topMargin()    { return (includeTopMargin    ? 17 : 0); }
    private int leftMargin()   { return (includeBorderMargin ? 12 : 0); }
    private int rightMargin()  { return (includeBorderMargin ? 11 : 0); }
    private int bottomMargin() { return (includeBorderMargin ? 11 : 0); }
	...
	public void layoutContainer(Container parent) {
		...
		if (size.width < minSize.width) {
			...
			for (AbstractButton button : buttons) {
				button.setBounds(atX, atY, buttonWidth, buttonHeight);
				atX += buttonWidth + 5;
			}
		}
    }
